# 🚀 Scaling: Overview  {background-color="#FFFFFF"}
...
# AI 🤝 Compute \[Historical\] {.centeredslide .smaller height="100%" background-color="#FFFFFF" auto-animate=true}
...
# Parallelism Concepts {.centeredslide .smaller}
...
- 🔗 See also:
  - [{{< iconify logos microsoft-icon >}} ZeRO-Offload](https://www.deepspeed.ai/tutorials/zero-offload/)
...
- Instead of maintaining per-GPU copy of `{params, grads, opt_states}`, FSDP shards (distributes) these across data-parallel workers
  - can optionall offload the sharded model params to CPU
:::
...
## Data Parallel Training {.centeredslide height="100%" background-color="#FFFFFF" auto-animate=true}
...
![](./assets/data-parallel-light.svg){#fig-data-parallel}

:::

## Data Parallel Training {background-color="#FFFFFF" auto-animate=true}
...
- Existing frameworks:
  ...
  - [DDP](https://pytorch.org/docs/stable/notes/ddp.html), etc)

- Recent presentation on "_Parallel Training Techniques_" on [🎬 YouTube](https://youtu.be/930yrXjNkgM)
:::

![](https://saforem2.github.io/distributed-training-slides/assets/data-parallel.svg)
...
## Deal with Data {background-color="#FFFFFF"}
...
- This can be done in one of two ways:
  ...
  ::: {.callout-warning icon=false title="⚠️  Warning" style="max-width:98%!important;"}
...
## Broadcast Initial State {background-color="#FFFFFF"}
...
## Best Practices {.smaller background-color="#FFFFFF"}
...
## Why Distributed Training? {background-color="#FFFFFF"}
...
## Recent Progress {background-color="#FFFFFF"}
...
## Deciding on a Parallelism Strategy  {background-color="#FFFFFF"}
...
- Largest layer **DOES NOT** fit on a single GPU:
  - `ZeRO` + Enable [Memory Centric Tiling (MCT)](https://deepspeed.readthedocs.io/en/latest/zero3.html#memory-centric-tiling)[^mct]
...
# Tensor (/ Model) Parallel Training: Example {background-color="#FFFFFF"}
...
## Model Parallel Training {background-color="#FFFFFF"}
...
This information is based on (the much more in-depth) [TP Overview](https://github.com/huggingface/transformers/issues/10321#issuecomment-783543530)
