...
1.  🐚 [Shell Utilities](#shell)
2.  🐍 [Python Library](#python)

## 🐚 Shell

> \[!IMPORTANT\]<br> We provide a variety of helper functions designed
> to make your life easier.
>
> They’re entirely self-contained in
> [**`utils.sh`**](../../ezpz/src/ezpz/bin/utils.sh) and are all
> prefixed with `ezpz_`
...
1.  `ezpz_setup_python`: This is done in two parts.

    1.  `ezpz_setup_conda`: Will first attempt to load and activate a
        suitable `conda` environment.

        Depending on where we’re running from, this will behave slightly
        differently:

        - **ALCF**[^1]: Automatically load the most recent `conda`
        ...
        - **Perlmutter**: Load the appropriate `pytorch` module and
          activate environment

        - **Unknown**: In this case, we will look for a `conda`,
          `mamba`, or `micromamba` executable, and if found, use that to
          activate the base environment.

    2.  `ezpz_setup_venv`: Ideally, we are now inside a suitable `conda`
        environment.
        ...
        If this exists, activate it and continue. Otherwise, we will
        create a new `venv` via: virtual
...
2.  Use `ezpz_setup_python` to:
    ...
    2.  Create[^4] a virtual environment *on top of* the base `conda`
        environment[^5]
...
[^1]: Any of {Aurora, Polaris, Sophia, Sunspot, Sirius}

[^2]: Note the `--require-virtualenv` isn’t *strictly* required, but I
    highly recommend to always try and work within a virtual
    environment, when possible.

[^3]: `deepspeed`, `DDP` only support `pytorch`

[^4]: If necessary, otherwise activate if already exists

[^5]: Note that the virtual environment will be created at
